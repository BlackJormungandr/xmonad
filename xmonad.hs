...
myWorkspaces    = ["web","code","term","discord","media","market","music"]
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm              , xK_o), sendMessage (Toggle NOBORDERS))
...
myLayout = avoidStruts (tiled ||| Mirror tiled ||| threeCol) ||| full
    where
      tiled = renamed [Replace "tall"] $ mySpacing 10 $ ResizableTall nmaster delta ratio []
      threeCol = renamed [Replace "threeCol"] $ mySpacing 10 $ ResizableThreeCol nmaster delta ratio []
      full = BO.lessBorders BO.Never Full 
...
main = do
  ...
  xmonad $ docks
         ...
         logHook = dynamicLogWithPP xmobarPP {
                ...
                , ppCurrent = xmobarColor "#F2F213" "" . wrap "{" "}"
         }
      }
